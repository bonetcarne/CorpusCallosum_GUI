...
%       for i=1:4, hCntr(i)=rectangle(circPrp{:},'Visible',vis);end
        hCntr(1)=rectangle(circPrp{:},'Visible',vis);
...
%         set(hCntr(i),'Position',[x-r y-r 2*r 2*r],'Visible','on'); % visible= off quita los circulos de control
%       end
    x=mean(xs(ids)); y=mean(ys(ids));
    set(hCntr(1),'Position',[x-r y-r 2*r 2*r],'Visible','on')
    end
  end
